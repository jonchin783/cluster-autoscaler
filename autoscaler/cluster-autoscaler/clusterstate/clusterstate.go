...
func getNotRegisteredNodes(allNodes []*apiv1.Node, cloudProviderNodeInstances map[string][]cloudprovider.Instance, time time.Time) []UnregisteredNode {
	...
	for _, node := range allNodes {
		registered.Insert(node.ObjectMeta.Name)
		//testing by jonachin
		//registered.Insert(node.Spec.ProviderID)
		klog.V(4).Infof("jonachin - Registered Nodes ID %s", node.Spec.ProviderID)
		klog.V(4).Infof("jonachin - objectMeta %s", node.ObjectMeta.Name)
	}
	klog.V(4).Infof("jonachin - cloudProviderNodeInstances %v", cloudProviderNodeInstances)

	notRegistered := make([]UnregisteredNode, 0)
	for _, instances := range cloudProviderNodeInstances {
		for _, instance := range instances {
			klog.V(4).Infof("jonachin - Instance ID %s", instance.Id)
	...
	klog.Infof("not registered %v", notRegistered)
