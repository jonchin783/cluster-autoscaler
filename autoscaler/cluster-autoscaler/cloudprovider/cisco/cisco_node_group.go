...
// ciscoNodeGroup implements NodeGroup interface from cluster-autoscaler/cloudprovider.
//
...
func (ng *ciscoNodeGroup) IncreaseSize(delta int) error {
	...
	size, err := ng.ciscoManager.nodeGroupSize(ng.id)
	if err != nil {
	...
	*ng.targetSize += delta
	...
	err = ng.ciscoManager.createNodes(ng.id, delta)
	if err != nil {
...
//
// The process of deletion depends on the implementation of ciscoManager,
...
func (ng *ciscoNodeGroup) DeleteNodes(nodes []*apiv1.Node) error {
	...
	if time.Since(ng.deleteNodesCachedSizeAt) > time.Second*10 {
		cachedSize, err = ng.ciscoManager.nodeGroupSize(ng.id)
		if err != nil {
	...
	err = ng.ciscoManager.deleteNodes(ng.id, nodeRefs, cachedSize-len(nodes))
	if err != nil {
		return fmt.Errorf("manager error deleting nodes: %v", err)
	}

	// Check the new node group size and store that as the new target
	newSize, err := ng.ciscoManager.nodeGroupSize(ng.id)
	if err != nil {
...
// DecreaseTargetSize decreases the cluster node_count in cisco container platform cluster.
func (ng *ciscoNodeGroup) DecreaseTargetSize(delta int) error {
	if delta >= 0 {
...
func (ng *ciscoNodeGroup) Nodes() ([]cloudprovider.Instance, error) {
	nodes, err := ng.ciscoManager.getNodes(ng.id)
	if err != nil {
...
func (ng *ciscoNodeGroup) TemplateNodeInfo() (*schedulerframework.NodeInfo, error) {
	return ng.ciscoManager.templateNodeInfo(ng.id)
}

// Exist returns if this node group exists.
// Currently always returns true.
func (ng *ciscoNodeGroup) Exist() bool {
	return true
}

// Create creates the node group on the cloud provider side.
func (ng *ciscoNodeGroup) Create() (cloudprovider.NodeGroup, error) {
	return nil, cloudprovider.ErrAlreadyExist
}

// Delete deletes the node group on the cloud provider side.
func (ng *ciscoNodeGroup) Delete() error {
	return cloudprovider.ErrNotImplemented
}

// Autoprovisioned returns if the nodegroup is autoprovisioned.
func (ng *ciscoNodeGroup) Autoprovisioned() bool {
	return false
}

// MaxSize returns the maximum allowed size of the node group.
func (ng *ciscoNodeGroup) MaxSize() int {
	return ng.maxSize
}

// MinSize returns the minimum allowed size of the node group.
func (ng *ciscoNodeGroup) MinSize() int {
	return ng.minSize
}

// TargetSize returns the target size of the node group.
func (ng *ciscoNodeGroup) TargetSize() (int, error) {
