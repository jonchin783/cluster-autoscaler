...
func TestIncreaseDecreaseSize(t *testing.T) {
	var m *ciscoManagerRest
	server := NewHttpServerMock()
	defer server.Close()
	assert.Equal(t, true, true)
	if len(os.Getenv("PACKET_AUTH_TOKEN")) > 0 {
		// If auth token set in env, hit the actual Packet API
		m = newTestCiscoManagerRest(t, "https://api.packet.net")
	} else {
		// Set up a mock Packet API
		m = newTestCiscoManagerRest(t, server.URL)
		server.On("handle", "/projects/"+m.projectID+"/devices").Return(listCiscoDevicesResponse).Times(4)
		server.On("handle", "/projects/"+m.projectID+"/devices").Return(listCiscoDevicesResponseAfterCreate).Times(2)
		server.On("handle", "/projects/"+m.projectID+"/devices").Return(listCiscoDevicesResponse)
	}
	...
	n2, err := ng.ciscoManager.getNodeNames(ng.id)
	assert.NoError(t, err)
